use crate::nft_core::tests::utils::{
    abi_calls::{balance_of, mint, owner_of, tokens_minted},
    test_helpers::setup,
};
...
mod success {
    ...
    async fn mints_multiple() {
        ...
        assert_eq!(owner_of(&owner1.contract, 0).await, Option::Some(minter.clone()));
        assert_eq!(owner_of(&owner1.contract, 1).await, Option::Some(minter.clone()));
        assert_eq!(owner_of(&owner1.contract, 2).await, Option::Some(minter.clone()));
        assert_eq!(owner_of(&owner1.contract, 3).await, Option::Some(minter.clone()));
    }
