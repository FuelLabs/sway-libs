use crate::bytecode::tests::utils::{
    abi_calls::{swap_configurables, test_function},
    test_helpers::{build_configurables, contract_bytecode, defaults, deploy_simple_contract_from_bytecode, predicate_bytecode, setup_predicate_from_bytecode, spend_predicate, test_contract_instance},
};
...
mod success {
    ...
    #[tokio::test]
    async fn swap_configurables_in_contract() {
        ...
        let result_bytecode = swap_configurables(&test_contract_instance, file_bytecode.clone(), my_configurables.clone()).await;
        ...
        let simple_contract_instance = deploy_simple_contract_from_bytecode(wallet, result_bytecode).await;
    ...
    async fn swap_configurables_in_predicate() {
        ...
        let predicate_instance = setup_predicate_from_bytecode(wallet.clone(), result_bytecode, config_value as u64).await;
