...
mod success {
    ...
    async fn compute_predicate_address_from_bytecode() {
        ...
        let result_address =
            compute_predicate_address(&test_contract_instance, file_bytecode, None).await;
    ...
    async fn compute_predicate_address_with_configurables_from_bytecode() {
        ...
        // Call the contract and compute the address
        let result_address = compute_predicate_address(
...
mod revert {
    ...
    #[tokio::test]
    #[should_panic]
    async fn when_bytecode_is_empty_with_configurables() {
        ...
        // Call the contract and compute the address
        let _result_address = compute_predicate_address(
            &test_contract_instance,
            empty_bytecode,
            Some(my_configurables),
        )
    ...
    async fn when_bytecode_is_empty() {
        ...
        // Call the contract and compute the address
        let _result_address =
            compute_predicate_address(&test_contract_instance, empty_bytecode, None).await;
    }
}
